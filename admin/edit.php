...
elseif($_GET['aksi']=='proseseditlayanan'){
	if (empty($_POST[jd]) || empty($_POST[isi])){
		echo "<script>window.alert('Data yang Anda isikan belum lengkap');
			   window.location=('javascript:history.go(-1)')</script>";
			}
...
elseif($_GET['aksi']=='proseseditqrcode'){
	require_once '../qrcode/qrlib.php';
	// Data yang akan dijadikan QR Code
	$url = $_POST['url']; // Ganti dengan data yang diinginkan
	...
	mysqli_query($koneksi,"UPDATE berita SET url='$url', status_url='sudah' WHERE id_berita='$_GET[id_berita]'");
echo "<script>window.location=('index.php?aksi=informasi')</script>";
}
///////////////////////////////////////////////////////////////////////////////////////////////////
